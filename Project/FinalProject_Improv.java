...
public class FinalProject_Improv
{
    ...
    {   
        ...
        ArrayList<ArrayList<String>> customerInfo = new ArrayList<>();     // 0 name, 1 Status, 2 PayMethod
        ArrayList<ArrayList<String>> customerOrders = new ArrayList<>();
        ArrayList<ArrayList<Integer>> customerQtys = new ArrayList<>();
        ArrayList<ArrayList<Integer>> customerPrices = new ArrayList<>();   // price per book
        ArrayList<ArrayList<Double>> breakDownAmount = new ArrayList<>();  // 0 VAT, 1 subtotal, 2 discount, 3 Total, 4 remaining balance
        ...
        displayHome(codeOfBooks1, quantityOfBooks1,priceOfBooks1,codeOfBooks2,quantityOfBooks2, priceOfBooks2,
                    ...
                    customerQtys, customerPrices, breakDownAmount, costumerCount, lengthOfCode);
    ...
    public static void displayHome(ArrayList<String> codeOfBooks1, ArrayList<Integer> quantityOfBooks1, ArrayList<Integer> priceOfBooks1, 
                                   ...
                                   int maxQuantity,int minQuantity, int maximumBookType, int maximumCustomers, 
                                 ArrayList<ArrayList<String>> customerInfo,
                                   ArrayList<ArrayList<String>> customerOrders,
                                   ArrayList<ArrayList<Integer>> customerQtys,
                                   ArrayList<ArrayList<Integer>> customerPrices,  
                                   ArrayList<ArrayList<Double>> breakDownAmount, int costumerCount, int lengthOfCode)
    {
        ...
        switch (inputDashBoard)
        {
            ...
            case "C": {
                ...
                while (costumerCount +1 <maximumCustomers)
                {
               int bookPrice = 0;
                int bookStock = 0;
               // int category = -1;
                int quantity = 0;// from user
                int numOrders = 0; //count of orders
                double initialPrice= 0, totalPrice=0, installment=0, remainBal=0; // needs to back to 0 after a customer

                double vat=0, totalItemVat=0.0, itemDiscount=0.0; // will be overwrite if being loop
                String bookName = "";
                String bookCategory ="0";
                
             
                ArrayList <String> forCustoInfo = new ArrayList();
                ArrayList <String> ordersList = new ArrayList();
                ArrayList <Integer> quantityList = new ArrayList();
                ArrayList <Integer> priceList = new ArrayList();
                ArrayList <Double> forBreakDownAmount = new ArrayList();
                
                 char ansToContinue='N';
                 boolean errorPrevention= false;
                    
                    System.out.print("Customer's Name: ");
                    forCustoInfo.add(0, in.nextLine());
                     
                    System.out.println("[R] Regular , [S] SENIOR, [P] PWD");
                    System.out.print("Customer's Status: ");
                    char custoStat = in.next().toUpperCase().charAt(0);
                    in.nextLine();
                    
                    if (custoStat=='R')
                    {
                        forCustoInfo.add(1, "Regular");
                        itemDiscount =0.0;
                        vat=0.12;
                    ...
                    {
                         forCustoInfo.add(1, "Senior Citizen");
                         itemDiscount =0.2;
                         vat=0.0;
                    }
                    ...
                    do
                    {
                        ...
                        System.out.print(numOrders+1 + ".) " + "Item's Code: ");
                        String codeItem = in.next().toUpperCase().trim();
                        
                        for(int ctr =0; ctr<codeOfBooks1.size(); ctr++)
                        {
                            if(codeItem.equalsIgnoreCase(codeOfBooks1.get(ctr).substring(0, 5)))
                            {
                                bookName = codeItem + " " + codeOfBooks1.get(ctr).substring(6, codeOfBooks1.get(ctr).length() - 9);
                                ...
                                if(quantity<= bookStock && quantity>0 )
                                {
                                  ordersList.add(numOrders, bookName);
                                 priceList.add(numOrders,priceOfBooks1.get(ctr));
                                 quantityList.add(numOrders, quantity);
                                  quantityOfBooks1.add(ctr, quantityOfBooks1.get(ctr) - quantity);
                                 errorPrevention= true;
                                System.out.print("Do another Order? [Y], [N]: ");
                                ansToContinue = in.next().toUpperCase().charAt(0);
                                numOrders++;
                                }
                        ...
                        for(int ctr =0; ctr<codeOfBooks2.size(); ctr++)
                        {
                            if(codeItem.equalsIgnoreCase(codeOfBooks2.get(ctr).substring(0, 5)))
                            {
                                bookName = codeItem + " " + codeOfBooks2.get(ctr).substring(6, codeOfBooks2.get(ctr).length() - 9);
                                ...
                                {
                                ordersList.add(numOrders, bookName);
                                priceList.add(numOrders,priceOfBooks2.get(ctr));
                                quantityList.add(numOrders, quantity);
                                quantityOfBooks2.add(ctr, quantityOfBooks2.get(ctr) - quantity);
                                errorPrevention= true;
                                System.out.print("Do another Order? [Y], [N]: ");
                                ansToContinue = in.next().toUpperCase().charAt(0);
                                numOrders++;
                        ...
                        for(int ctr = 0; ctr<codeOfBooks3.size(); ctr++)
                        {
                            if(codeItem.equalsIgnoreCase(codeOfBooks3.get(ctr).substring(0, 5)))
                            {
                                bookName = codeItem + " " + codeOfBooks3.get(ctr).substring(6, codeOfBooks3.get(ctr).length() - 9);
                                ...
                                {
                                    ordersList.add(numOrders, bookName);
                                    priceList.add(numOrders,priceOfBooks3.get(ctr));
                                 quantityList.add(numOrders, quantity);
                                  quantityOfBooks3.add(ctr, quantityOfBooks3.get(ctr) - quantity);
                                 errorPrevention= true;
                                System.out.print("Do another Order? [Y], [N]: ");
                                ansToContinue = in.next().toUpperCase().charAt(0);
                                numOrders++;
                        ...
                        for(int ctr = 0; ctr<codeOfBooks4.size(); ctr++)
                        {
                            if(codeItem.equals(codeOfBooks4.get(ctr).substring(0, 5)))
                            {
                                bookName = codeItem + " " + codeOfBooks4.get(ctr).substring(6, codeOfBooks4.get(ctr).length() - 9);
                                ...
                                {
                                     ordersList.add(numOrders, bookName);
                                    priceList.add(numOrders, priceOfBooks4.get(ctr));
                                 quantityList.add(numOrders, quantity);
                                 quantityOfBooks4.add(ctr, quantityOfBooks4.get(ctr) - quantity);
                                 errorPrevention= true;
                                System.out.print("Do another Order? [Y], [N]: ");
                                ansToContinue = in.next().toUpperCase().charAt(0);
                                numOrders++;
                        ...
                        /*if (!bookCategory.equalsIgnoreCase(""))
                        {
                            System.out.print("Quantity: ");
                            quantity = in.nextInt();
                        ...
                        if (bookCategory.equalsIgnoreCase("0"))
                            System.out.println("Code is not Found");
                        
                    } while (ansToContinue == 'Y' || bookCategory.equals("0") || errorPrevention == false);
                    
                    // computation ------------------------------------------------------------------------------------
                    
                    
                    for(int ctr =0; ctr <numOrders; ctr++)                                          //total VAT and initial Price Computation
                    {
                        totalItemVat = totalItemVat + (quantityList.get(ctr) * priceList.get(ctr) * vat);
                        System.out.println("VAT: " + totalItemVat);
                        initialPrice += quantityList.get(ctr) * priceList.get(ctr); 
                    }
                    
                     int totalQty = 0;
                        for (int q : quantityList) {
                            totalQty += q;
                        }
                        
                    System.out.println("Initial Amount: " + initialPrice);
                    System.out.println("VAT: " + totalItemVat);
                    System.out.println("-------------------------------------------------------------------------");
                    
                    forBreakDownAmount.add(totalItemVat); //0
                    forBreakDownAmount.add(initialPrice); //1
                    
                   
                    System.out.println("[C] CASH , [I] INSTALLMENT");
                    System.out.println("Payment Method: ");
                    char payMethInfo = in.next().toUpperCase().charAt(0);
                    
                     if (payMethInfo=='C')
                     {
                        forCustoInfo.add("Cash");
                        if (totalQty >= 3)
                            itemDiscount += 0.05;
                        
                         System.out.println("DISCOUNT: " + itemDiscount );
                        forBreakDownAmount.add(itemDiscount); //2
                        totalPrice = initialPrice - (initialPrice * itemDiscount);
                        totalPrice +=totalItemVat;
                        forBreakDownAmount.add( totalPrice);//3
                        forBreakDownAmount.add(installment);//4
                        forBreakDownAmount.add(remainBal);//5
                     }
                   
                    else if(payMethInfo=='I')
                    {
                         forCustoInfo.add("Installment");
                          System.out.println("DISCOUNT: " + itemDiscount );
                         totalPrice = initialPrice - (initialPrice * itemDiscount);
                         totalPrice +=totalItemVat;
                         forBreakDownAmount.add(itemDiscount); //2
                         ...
                         installment = totalPrice* .60;
                         forBreakDownAmount.add(installment);//4
                         remainBal = totalPrice - installment;
                         forBreakDownAmount.add(remainBal);//5
                         //System.out.println("First Installment: " + installment);
                         //System.out.println("Remaining Balance: " +remainBal);
                    }
                     
                     
                    if(forCustoInfo.get(2).equalsIgnoreCase("Cash"))
                {
                    System.out.println(ordersList +"| " + quantityList+ "| " + priceList );
                     System.out.print("\n============================== RECEIPT ================================\n");
                     System.out.print("Customer #" + (costumerCount+1) + ": " + forCustoInfo.get(0) + "\n");
                     System.out.printf("%-35s %11s %9s %12s\n", "Book Title", "Qty", "Price", "Subtotal" + "\n");
                     System.out.print("-----------------------------------------------------------------------\n");
                    for (int j = 0; j < ordersList.size(); j++) {                                                                               //hereeee
                       int comPerOrder =  quantityList.get(j)*  priceList.get(j);
                       System.out.printf("%-35s %9d %9d %9d\n", ordersList.get(j), quantityList.get(j), priceList.get(j), comPerOrder);
                    }
                    System.out.print("-----------------------------------------------------------------------\n");
                   
                     System.out.print("Total Quantity Purchased:\t\t\t " + totalQty + " item(s)" + "\n");
                     System.out.print("Initial Amount          :\t\t\t Php " + df.format(forBreakDownAmount.get(1))+ "\n");
                     System.out.print("Discount                :\t\t\t Php " + df.format(forBreakDownAmount.get(2))+ "\n");
                     System.out.print("VAT                     :\t\t\t Php " + df.format(forBreakDownAmount.get(0))+ "\n");
                     System.out.print("Total                   :\t\t\t Php " + df.format(forBreakDownAmount.get(3))+ "\n");
                     System.out.print("=======================================================================\n");
                }
                
                    else if(forCustoInfo.get(2).equalsIgnoreCase("Installment"))
                    {
                    System.out.print("\n============================== RECEIPT ================================\n");
                    System.out.print("\n============================== INSTALLMENT ================================\n");
                    System.out.print("Customer #" + (costumerCount+1) + ": " + forCustoInfo.get(0) + "\n");
                    ...
                    System.out.print("Total                   :\t\t\t Php " + df.format(forBreakDownAmount.get(3))+ "\n");
                    System.out.print("=======================================================================\n");
                    System.out.print("First Installment       :\t\t\t\tPhp " + df.format(forBreakDownAmount.get(4))+ "\n");
                    System.out.print("Remaining Balance       :\t\t\t\tPhp " +df.format(forBreakDownAmount.get(5))+ "\n");
                    System.out.print("=======================================================================\n");
                    }
                
                    receipt( maximumCustomers, costumerCount, numOrders, forCustoInfo, ordersList, quantityList,
                                     priceList, forBreakDownAmount);
                    System.out.println("Customer Count: " + costumerCount);
                   //b4 confirmation
                    customerInfo.add(forCustoInfo);
                    customerOrders.add(ordersList);
                    customerQtys.add(quantityList);
                    customerPrices.add(priceList);
                    breakDownAmount.add(forBreakDownAmount);
                    
                    forCustoInfo.clear();
                    ordersList.clear();
                    quantityList.clear();
                    priceList.clear();
                    forBreakDownAmount.clear();
                    
                    System.out.println("-------------------------------------------------------------------------");
                    System.out.print("Another Customer? [Y], [N]: ");
                    char anotherCusto = in.next().toUpperCase().charAt(0);
                    in.nextLine();
...
// pabago yung location ng file
    public static void receipt(int maximumCustomers, int costumerCount, int numOrders, 
                                     ArrayList <String> forCustoInfo,
                                    ArrayList <String> ordersList,
                                    ArrayList <Integer> quantityList,
                                    ArrayList <Integer> priceList,
                                    ArrayList <Double> forBreakDownAmount)
    {
            DecimalFormat df = new DecimalFormat("0.00");
            try{
        ...
        ArrayList<ArrayList<Integer>> customerQtys = new ArrayList<>();
        ArrayList<ArrayList<Integer>> customerPrices = new ArrayList<>();   // price per book
        ArrayList<ArrayList<Double>> breakDownAmount = new ArrayList<>();  // 0 VAT, 1 subtotal, 2 discount, 3 Total, 4 installment, 5 remaining
                ...
                {
                     receipt.write("\n============================== RECEIPT ================================\n");
                     receipt.write("Customer #" + (costumerCount+1) + ": " + forCustoInfo.get(0) + "\n");
                    ...
                    int totalQty = 0;
                     ...
                     receipt.write("Initial Amount          :\t\t\t Php " + df.format(forBreakDownAmount.get(1))+ "\n");
                     receipt.write("Discount                :\t\t\t Php " + df.format(forBreakDownAmount.get(2))+ "\n");
                     receipt.write("VAT                     :\t\t\t Php " + df.format(forBreakDownAmount.get(0))+ "\n");
                     receipt.write("Total                   :\t\t\t Php " + df.format(forBreakDownAmount.get(3))+ "\n");
                     receipt.write("=======================================================================\n");
                }
                    ...
                    receipt.write("Total                   :\t\t\t Php " + df.format(forBreakDownAmount.get(3))+ "\n");
                    receipt.write("=======================================================================\n");
                    receipt.write("First Installment       :\t\t\t\tPhp " + df.format(forBreakDownAmount.get(4))+ "\n");
                    receipt.write("Remaining Balance       :\t\t\t\tPhp " +df.format(forBreakDownAmount.get(5))+ "\n");
                    receipt.write("=======================================================================\n");
                    }
